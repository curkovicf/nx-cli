...
export class WorkspacesController implements IController {
  ...
  private initValidateWorkspacePath(): void {
    ipcMain.on(
      WorkspacesIpcEvents.validateWorkspacePath.fromAngular,
      async (event, workspacePath: string) => {
        event.returnValue = await this.workspacesService.validateWorkspacePath(
          workspacePath,
        );
      },
    );
  ...
  private initGetAllTags(): void {
    ipcMain.on(
      WorkspacesIpcEvents.tags.fromAngular,
      async (event, workspacePath: string) => {
  ...
  private initGetNxGenerators(): void {
    ipcMain.on(
      WorkspacesIpcEvents.getAvailableGenerators.fromAngular,
      async (event, workspacePath: string) => {
        const response: IpcResponses.ResponseWithData<GeneratorsDto> =
