...
export const workspacesReducer = createReducer(
  initialState,
  on(WorkspacesActions.setWorkspacesState, (state, {workspacesState}) => ({
    ...workspacesState,
  })),
  on(WorkspacesActions.switchCurrentWorkspace, (state, {selectedWorkspace}) => ({
    ...state,
    selectedWorkspace,
  })),
  ...
  on(WorkspacesActions.addLogs, (state, {workspacePath, logs}) => ({
    selectedWorkspace:
      state.selectedWorkspace && state.selectedWorkspace.path === workspacePath
        ? {
            ...state.selectedWorkspace,
            consoleLogs: [...state.selectedWorkspace.consoleLogs, ...logs],
          }
        : state.selectedWorkspace,
    workspaces: [
      ...state.workspaces.map(w =>
        w.path === workspacePath ? {...w, consoleLogs: [...w.consoleLogs, ...logs]} : w,
      ),
    ],
  })),
  on(WorkspacesActions.clearLog, (state, {workspacePath}) => ({
    selectedWorkspace:
      state.selectedWorkspace.path === workspacePath
        ? {...state.selectedWorkspace, consoleLogs: []}
        : state.selectedWorkspace,
    workspaces: [
      ...state.workspaces.map(w =>
        w.path === workspacePath ? {...w, consoleLogs: []} : w,
      ),
    ],
  })),
  ...
  on(WorkspacesActions.addNxGenerators, (state, {nxGenerators: nxGeneratorsDto}) => ({
    workspaces: state.workspaces.map(w =>
      w.path === nxGeneratorsDto.workspacePath
        ? {
            ...w,
    ...
    selectedWorkspace:
      state.selectedWorkspace.path === nxGeneratorsDto.workspacePath
        ? {
            ...state.selectedWorkspace,
            generators: nxGeneratorsDto.generators,
          }
  ...
  on(WorkspacesActions.setSelectedProject, (state, {workspacePath, selectedProject}) => ({
    ...state,
    workspaces: state.workspaces.map(w =>
      w.path === workspacePath ? {...w, selectedProject} : w,
    ),
    selectedWorkspace: {
      ...state.selectedWorkspace,
      selectedProject,
